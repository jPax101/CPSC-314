	...
	Scene const &scene)
{
	...
	Vector viewVector = (scene.camera.center - scene.camera.position).normalized();
	Vector uVector = viewVector.cross(scene.camera.up);
	uVector.normalize();
	double distance = scene.camera.zNear;
	double buttom = -distance * tan(deg2rad(scene.camera.fovy * 0.5));
	double left = buttom * scene.camera.aspect;
	Vector buttomLeft = scene.camera.position + viewVector * distance + left * uVector + buttom * scene.camera.up;
	...
	for (int y = 0; y < scene.resolution[1]; y++) {
		for (int x = 0; x < scene.resolution[0]; x++) {
			...
			else
			{
				...
				Vector xIncVector = (uVector * 2 * (-left)) / scene.resolution[0];
				Vector yIncVector = (scene.camera.up * 2 * (-buttom)) / scene.resolution[1];
				ray = Ray(scene.camera.position, (buttomLeft + x*xIncVector + y*yIncVector - scene.camera.position).normalized());
	...
	else
	{
		...
		for (int x = 0; x< scene.objects.size(); x++) {
			if (scene.objects[x]->intersect(ray, hit)) {
				...
				outColor = shade(ray, rayDepth, hit, scene.objects[objNum]->material, scene);
			}
		}
		depth = minDepth;
		return true;
	...
	Scene const &scene)
{
	...
	for (auto lightIter = scene.lights.begin(); lightIter != scene.lights.end(); lightIter++)
	{
		...
		Ray shadowRay = Ray(intersection.position + (lightIter->position - intersection.position)* 1e-6,
